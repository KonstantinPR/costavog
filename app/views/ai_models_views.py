...
def train_model():
    ...
    if request.method == 'POST':
        df_predict = ai_models_module.xgb()
...
def train_model2():
    ...
    if request.method == 'POST':
        ...
        print(data_train)
        model = ai_models_module.train(data_train, encoded_col_name, goal_col)
...
def predict_by_model():
    ...
    if request.method == 'POST':
        ...
        data_test = ai_models_module.nan_correction(data_test)
        print(f"data_test {data_test}")
        data_train = ai_models_module.nan_correction(data_train)
        data_train, default_label_encoder = ai_models_module.mapping_categorical(data_train, encoded_col_name)
        data_test = ai_models_module.redundant_col_name(data_train, data_test)
        print(f"data_test {data_test}")
        data_predict = ai_models_module.predict(model, data_test, encoded_col_name, goal_col, default_label_encoder)

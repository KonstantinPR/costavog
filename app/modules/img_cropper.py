...
def set_parameters_via_type_clothes(type_clothes_to_crop):
    ...
    global K_BOTTOM_RIGHT_START
    global STEP_ITERATION
    global STEP_ITERATION_BOTTOM
    global SENSIBILITY_COLOR
    global COUNT_STEP_J
    global DELIMITER
    global NEW_HEIGHT_IM
    global K_WIDTH_HEIGHT_IM
    global WHITE_BLOCK_WIDTH
    global WHITE_BLOCK_HEIGHT
    global K_BOOT_LONG_LEGS

    if type_clothes_to_crop == "Одежда по пояс":
        # half-full clothes
    ...
    elif type_clothes_to_crop == "Обувь без модели":
        # shoes boot
...
def crop_images(images: Union[list[PIL.JpegImagePlugin.JpegImageFile], list], type_clothes_to_crop) -> BytesIO:
    set_parameters_via_type_clothes(type_clothes_to_crop)
    images_zipped = None
    images_set = []
    print(images)
    for img in images:
